...
def generate_csv():
    ...
    train_df, test_df, train_targets, test_targets = train_test_split(ionosphere_x_df, ionosphere_y_df, test_size=0.4,
                                                                      stratify=ionosphere_y_df)
    train_df.to_csv('folds\\train_df.csv', index=False)
    test_df.to_csv('folds\\test_df.csv', index=False)
    train_targets.to_csv('folds\\train_targets.csv', index=False)
    test_targets.to_csv('folds\\test_targets.csv', index=False)
